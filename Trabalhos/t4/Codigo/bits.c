...
// funções auxiliares para e percurso de bits
...
// as funções abaixo funcionam a partir dessa posição
...
// em um percurso de bits em b, retorna os próximos n bits, e avança esse tanto.
...
// tem uma figura na posição v0, com largura dv0 e outra em v1 com largura dv1
// ajusta v0 para a primeira posicão da figura 0 que coincide com a fig 1,
// v1 para a primeira posição em fig 1 que coincide com fig 0, e dv1 para
// o número de bits que coincidem
...
{
  if (x + o->largura < 0) return false;
  if (y + o->altura < 0) return false;
  if (b->largura < x) return false;
  if (b->altura < y) return false;
  return true;
}
...
bool b_colide(Bits b, int x, int y, Bits d)
{
  ...
  int bits_por_linha = d->largura;
  ajusta(&primeiro_bit_b, b->largura, &primeiro_bit_d, &bits_por_linha);
  if (bits_por_linha < 1) return false;

  int primeira_linha_b = y;
  int primeira_linha_d = 0;
  int nlinhas = d->altura;
  ajusta(&primeira_linha_b, b->altura, &primeira_linha_d, &nlinhas);
  if (nlinhas < 1) return false;
...
// nos códigos \u1fb00-3b (utf8 11110000 10011111 10101100 10xxxxxx)
// bits              unicode
// 000000          - ' '
